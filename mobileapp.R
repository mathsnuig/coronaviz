  ...
  dataGrowth <- reactive({
    ...
    output$newcases <- renderPlot({
      ...
      dataIreland() %>%
        ...
        ggplot(aes(x=Date,y=New_cases,fill=type)) + geom_col() + 
    ...
    output$dailyicu <- renderPlot({
      lims <- as.POSIXct(strptime(c("29/02/2020 00:00",paste0(daily_date, " 00:00")), format = "%d/%m/%Y %H:%M"))    
